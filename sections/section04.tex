...
Our model works on the systematic calling of timers when required. Some of these timers follow periodic patterns while others are called when certain conditional elements are encountered. For the conditional timers, the timer is called once by \textit{Timer.StartOneShot} method and this calling repeats in a loop until the condition to execute the loop holds. In the following subsections we will describe these timers and conditional elements to keep these timers running.
	...
	\begin{algorithm}
		\caption{\ac{PC} Advertisement}
		\label{BeaconTimer}
		\begin{algorithmic}[1]
			\Procedure{BeaconTimer}{}
			\State $sum = 0$
	...
	% BeaconTimer Sending Algorithm
	\begin{algorithm}
		\caption{\ac{PC} availability announcement in one hop by \acp{PC}}
		\label{BeaconTimerSend}
		\begin{algorithmic}[1]
			\Procedure{BeaconTimer.fired}{}
		
			\If {$not \textit{sendBeaconBusyOnehop}$}
			    \State $SendBeacon1hop(\ac{ETX}, TOS\_NODE\_ID)$
		    \EndIf	
		\EndProcedure
		\end{algorithmic}
	\end{algorithm}
	
	% BeaconTimer Reception Algorithm
	\begin{algorithm}
		\caption{\ac{PC} availability relaying in two hop by one hop recipients}
		\label{BeaconTimerReceive}
		\begin{algorithmic}[1]
			\Procedure{BeaconTimer.Receive}{}
			\If {$not \textit{sendBeaconBusyTwohop}$}
			    \State $SendBeacon2hop(\ac{ETX}, TOS\_NODE\_ID)$
			    \State $InsertIntoProcessingCentreStructArray(\ac{ETX}, TOS\_NODE\_ID)$
		    \EndIf	
		\EndProcedure
		\end{algorithmic}
	\end{algorithm}
	...
	% Insert the algorithm
	\begin{algorithm}
		\caption{Retrieve an optimal \ac{PC} for transmission}
		\label{getProcessingCentreForTransmission}
		\begin{algorithmic}[1]
			\Procedure{GetProcessingCentreForTransmission}{}
			\For {each \ac{PC} $i$ in $\ac{PC}StructArray$}
	            \For {each \ac{Busy\ac{PC}} $j$ in $Busy\ac{PC}StructArray$}
                    \If {$\textit{\ac{PC}}(i) != \textit{BusyProcessingCentre}(j)$}
                        \Return i
                    \EndIf
                \EndFor
		    \EndFor
